...
def my_plot(pred_test,pred_train,y_test,y_train,sup_title):
    ...
    plt.minorticks_on()
    plt.grid(1,'both')
    plt.title('Test : $\pm${0} : {1:.3f}%'.format(check,100*sum([np.abs(x)<check for x in err_test])/len(err_test)))
    
    plt.subplot(2,2,4)
    plt.hist(err_train, bins = 150)
    plt.minorticks_on()
    plt.grid(1,'both')
    plt.title('Train : $\pm${0} : {1:.3f}%'.format(check,100*sum([np.abs(x)<check for x in err_train])/len(err_train)))
